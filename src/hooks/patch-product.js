...
module.exports = function (options = {}) { // eslint-disable-line no-unused-vars
  return async context => {
    ...
    // The logged in user and product id
    const { _id, params } = context;
    const productService = context.app.service('product');
    const oldRecord = await productService.find({query: { id: _id }});
    ...
    notification.author = Object.entries(author)[0] && oldAuthor !== author ? `${data.author.name} was named author of the product ${name}` : '';
